...
ScCudaTorch = load(
    name="sc_cuda_torch",
    sources=["ScCudaTorch2.cpp", "ScCudaTorch_UniPinConst_2.cu"],
    verbose=True
)
...
class_name = 'Sneaker'
print("UNIPOLARS COMPACT: ", class_name)
...
for images in images_list:
    ...
    custom_output = Sc_Conv2_py(custom_weights, custom_biases, images, h_INPUT, h_KERNEL, input_channels, output_channels, bitstream_Length)
    #print(" ")
    ...
    new_weightsfc1 = torch.flatten(net.fc1.weight.data.t(), 0)
    new_weightsfc1 = new_weightsfc1.tolist()
    ...
    start_time = time.time()
    #print(" ")
    output_sc_FcLayer1 = torch.tensor(ScCudaTorch.ScCudaFcLayer(sc_flattened_list, new_weightsfc1, new_biasfc, new_INPUTfc_rndomMatrix, new_weightsfc_rndomMatrix, new_biasfc_rndomMatrix, noOutputs, bitstream_Length))
    #print(f"Execution time: {(time.time() - start_time):.2f} seconds")
    output_sc_FcLayer1 = (output_sc_FcLayer1/(no_accumulations*bitstream_Length))*no_accumulations
    ...
    new_weightsfc2 = torch.flatten(net.fc2.weight.data.t(), 0)
    new_weightsfc2 = new_weightsfc2.tolist()
    ...
    start_time = time.time()
    #print(" ")
    output_sc_FcLayer2 = torch.tensor(ScCudaTorch.ScCudaFcLayer(output_sc_FcLayer1_tensor_norm, new_weightsfc2, new_biasfc, new_INPUTfc_rndomMatrix, new_weightsfc_rndomMatrix, new_biasfc_rndomMatrix, noOutputs, bitstream_Length))
    #print(f"Execution time: {(time.time() - start_time):.2f} seconds")
    output_sc_FcLayer2 = (output_sc_FcLayer2/(no_accumulations*bitstream_Length))*no_accumulations
    ...
    new_weightsfc3 = torch.flatten(net.fc3.weight.data.t(), 0)
    ...
    start_time = time.time()
    #print(" ")
    output_sc_FcLayer3 = torch.tensor(ScCudaTorch.ScCudaFcLayer(output_sc_FcLayer2_tensor_norm, new_weightsfc3, new_biasfc, new_INPUTfc_rndomMatrix, new_weightsfc_rndomMatrix, new_biasfc_rndomMatrix, noOutputs, bitstream_Length))
    #print(f"Execution time: {(time.time() - start_time):.2f} seconds")
    output_sc_FcLayer3 = (output_sc_FcLayer3/(no_accumulations*bitstream_Length))*no_accumulations
